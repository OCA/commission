...
class TestSaleCommissionPricelist(SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.pricelist_version = cls.env['product.pricelist.version'].create({
            'pricelist_id': cls.pricelist.id,
            'name': 'Test commission pricelist version',
            'items_id': [
                (0, 0, {
    ...
    def test_change_sale_pricelist(self):
        ...
        self.pricelist_version_3 = self.env['product.pricelist.version'].create({
            'pricelist_id': pricelist_3.id,
            'name': 'Test commission pricelist version 3',
            'items_id': [
                (0, 0, {
        ...
        # Nothing changes
        self.sale_order.pricelist_id = pricelist_3.id
        self.assertEqual(
            self.so_line1.agents[:1].commission, self.commission_1)
        self.assertEqual(
            self.so_line2.agents[:1].commission, self.commission_2)
