...
class SaleCommissionLineMixin(models.AbstractModel):
    ...
    def _get_commission_amount(self, commission, subtotal, product, quantity, discount):
        # Method replaced
        self.ensure_one()
        ...
        for item_id in item_ids:
            ...
            if commission_item.discount_from + commission_item.discount_to > 0:
                if commission_item.discount_from <= discount < commission_item.discount_to:
                    break
            else:
                break
            commission_item = False
        if not commission_item:
            return 0.0, False  # all commissionitems was rejected because of dicount condition
        ...
        for item in self:
            if item.categ_id and item.applied_on == '2_product_category':
                ...
                item.name = _("Variant: %s") % (
                    item.product_id.with_context(display_default_code=False
                                                 ).display_name
                )
            else:
