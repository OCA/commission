...
class TestSaleCommission(SavepointCase):
    ...
    def test_sale_commission_net_amount_payment(self):
        sale_order = self._create_sale_order(self.agent_semi, self.commission_net_paid)
        sale_order.action_confirm()
        self.assertEqual(
            len(sale_order.invoice_ids),
            0,
    ...
    def test_sale_default_agent(self):
        sale_agent = self.env.ref("sale_commission.res_partner_pritesh_sale_agent")
        ...
        sale_order.order_line[0].agents = [
            (
                ...
                0,
                {
                    ...
                    "commission": self.env.ref("sale_commission.demo_commission").id,
                },
            ),
            (
                0,
    ...
    def test_check_new_invoice_with_settle_invoiced(self):
        self.check_new_invoice_with_settle_invoiced(
            self.agent_monthly, self.commission_section_invoice, 1
        )
    ...
    def test_join_settlements(self):
        ...
        with patch.object(
            Settlement, "create_invoice_header", return_value=agent_invoice
        ):
    ...
    def check_new_invoice_with_settle_invoiced(self, agent, commission, period):
        sale_order = self._create_sale_order(agent, commission)
        sale_order.action_confirm()
        self.assertEqual(len(sale_order.invoice_ids), 0)
        payment = self.advance_inv_model.create(
