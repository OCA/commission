...
class res_partner_agent(osv.osv):
    ...
    def _get_partner_agents_to_update_from_sale_agents(self, cr, uid, ids, context=None):
        """devuelve los ids de partner agents a actualizar desde el lanzamiento de un evento de actualizaci??n en agentes de ventas"""
        if context is None: context = {}
        result = []
    ...
    def onchange_agent_id(self, cr, uid, ids, agent_id):
        """al cambiar el agente cargamos sus comisi??n"""
        result = {}
        v = {}
        if agent_id:
    ...
    def onchange_commission_id(self, cr, uid, ids, agent_id=False, commission_id=False):
        """al cambiar la comisi??n comprobamos la selecci??n"""
        result = {}
