...
class InvoiceLineAgent(models.Model):
    """invoice agents"""
    ...
    commission_id = fields.Many2one("commission",
                                    required=True, ondelete="cascade")
    settled = fields.Boolean(string="Settled", readonly=True, default=False)
    ...
    def calculate_commission(self):
        for line_agent in self:
            ...
            if commission_type == 'fixed' and line_agent.commission_id.fix_qty:
                ...
                line_agent.quantity = subtotal * (fix_qty / 100.0)
...
class AccountInvoiceLine(models.Model):
    ...
    commission_ids = fields.One2many(
        "invoice.line.agent", "invoice_line_id", string="Commissions",
        help="Commissions related to invoice line.")
...
class AccountInvoice(models.Model):
    ...
    agent_code = fields.Char(string="Agent code", related="agent_id.code",
                             readonly=True)
    country = fields.Many2one("res.country",
                              related="partner_id.country_id", readonly=True)

    @api.multi
    def onchange_partner_id(self, type, part, date_invoice=False,
                            payment_term=False, partner_bank_id=False,
                            company_id=False):
        """When changing partner get related agent"""
        res = super(AccountInvoice, self).onchange_partner_id(
            type, part, date_invoice, payment_term, partner_bank_id,
            company_id)
        if part and res.get('value', False):
            partner_obj = self.env['res.partner']
            partner = partner_obj.browse(part)
            if partner.agent_ids:
                agent_id = partner.agent_ids[0].agent_id.id
