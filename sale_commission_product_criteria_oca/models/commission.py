    ...
    def _compute_commission_item_name_value(self):
        for item in self:
            if item.categ_id and item.applied_on == "2_product_category":
                ...
                item.name = self.env._("Product: %s", item.product_tmpl_id.display_name)
            ...
            if item.commission_type == "fixed":
                ...
                amount_repr = float_repr(item.fixed_amount, decimal_places)
                symbol = item.currency_id.symbol or ""
                if item.currency_id.position == "after":
                    item.commission_value = f"{amount_repr} {symbol}"
                else:
                    item.commission_value = f"{symbol} {amount_repr}"
